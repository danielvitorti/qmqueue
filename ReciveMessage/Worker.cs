        ...
        protected override async Task ExecuteAsync(CancellationToken stoppingToken)
        {
            while (!stoppingToken.IsCancellationRequested)
            {
                ...
                using (SQLiteConnection connection = new SQLiteConnection(connectionString))
                {
                    connection.Open();
                    try{
                    string selectQuery = "SELECT * FROM MENSAGEM_ENTRADA_CABECALHO WHERE STATUS = 'N'";
                    using (SQLiteCommand command = new SQLiteCommand(selectQuery, connection))
                    {
                        using (SQLiteDataReader reader = command.ExecuteReader())
                        {
                            while (reader.Read())
                            {
                                ...
                                string insertQuery = "INSERT INTO MENSAGEM_ENTRADA_CABECALHO_NOVO (SISTEMA_ORIGEM, ID_MENSAGEM, SISTEMA_DESTINO, ...) VALUES (@SISTEMA_ORIGEM, @ID_MENSAGEM, @SISTEMA_DESTINO, ...)";
                                ...
                                string updateQuery = $"UDPATE MENSAGEM_ENTRADA_CABECALHO SET status = 'P' "+
